...
namespace LiteDB
{
    ...
    // if page is empty, convert to T
            ...
            where T : BasePage
        {
            ...
            var header = GetPage<HeaderPage>(0, true);
            uint pageID = 0;
        ...
        public void DeletePage(uint pageID, bool addSequence = false)
        {
            ...
            var pages = addSequence ? GetSeqPages<BasePage>(pageID).ToArray() : new[] {GetPage<BasePage>(pageID)};
            ...
            while (nextPageID != uint.MaxValue)
            {
                ...
                if (free >= next.FreeBytes)
                {
                    ...
                    else
                    {
                        ...
                        var prev = GetPage<BasePage>(page.PrevPageID, true);
        ...
        private void RemoveToFreeList(BasePage page, BasePage startPage, ref uint fieldPageID)
        {
            ...
            else
            {
                ...
                var prevPage = GetPage<BasePage>(page.PrevPageID, true);
            ...
            {
                var nextPage = GetPage<BasePage>(page.NextPageID, true);
