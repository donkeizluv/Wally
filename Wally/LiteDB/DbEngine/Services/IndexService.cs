...
namespace LiteDB
{
    ...
    internal class IndexService
    {
        ...
        public CollectionIndex CreateIndex(CollectionPage col)
        {
            ...
            // insert tail node
            var tail = AddNode(index, BsonValue.MaxValue, IndexNode.MAX_LEVEL_LENGTH);
        ...
        public IndexNode AddNode(CollectionIndex index, BsonValue key)
        {
            ...
            return AddNode(index, key.Normalize(index.Options), FlipCoin());
        }
        ...
        {
            // calc key size
            int keyLength = key.GetBytesCount(false);
            ...
            for (int i = IndexNode.MAX_LEVEL_LENGTH - 1; i >= 0; i--)
            {
                ...
                for (; cur.Next[i].IsEmpty == false; cur = cache)
                {
                    ...
                    int diff = cache.Key.CompareTo(key);
            ...
            _pager.AddOrRemoveToFreeList(page.FreeBytes > IndexPage.INDEX_RESERVED_BYTES, page, index.Page,
                ref index.FreeIndexPageID);
        ...
        public void Delete(CollectionIndex index, PageAddress nodeAddress)
        {
            ...
            else
            {
                ...
                _pager.AddOrRemoveToFreeList(page.FreeBytes > IndexPage.INDEX_RESERVED_BYTES, node.Page, index.Page,
                    ref index.FreeIndexPageID);
            }
        }
        ...
        /// <summary>
        ///     Find first node that index match with value. If not found but sibling = true, returns near node (only non-unique
        ///     index)
        ...
        public IndexNode Find(CollectionIndex index, BsonValue value, bool sibling, int order)
        {
            ...
            for (int i = IndexNode.MAX_LEVEL_LENGTH - 1; i >= 0; i--)
            {
                for (; cur.NextPrev(i, order).IsEmpty == false; cur = GetNode(cur.NextPrev(i, order)))
                {
                    ...
                    int diff = next.Key.CompareTo(value);
