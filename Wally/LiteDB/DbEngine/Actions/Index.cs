        ...
        public IEnumerable<BsonDocument> GetIndexes(string colName, bool stats = false)
        {
            ...
            foreach (var index in col.GetIndexes(true))
            {
                ...
                if (stats)
                {
                    ...
                    int pages = _indexer.FindAll(index, Query.Ascending).GroupBy(x => x.Page.PageID).Count();

                    // this command can be consume too many memory!! has no CheckPoint on loop
                    double keySize = pages == 0 ? 0 : _indexer.FindAll(index, Query.Ascending).Average(x => x.KeyLength);
