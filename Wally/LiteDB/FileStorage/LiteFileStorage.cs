...
namespace LiteDB
{
    ...
    /// </summary>
    internal class LiteFileStorage
    {
        ...
        public bool Delete(string id)
        {
            ...
            int d = _engine.Delete(FILES, Query.EQ("_id", id));
            ...
            while (true)
            {
                int del = _engine.Delete(CHUNKS, Query.EQ("_id", LiteFileInfo.GetChunckId(id, index++)));
        ...
        {
            return Upload(new LiteFileInfo(id), stream);
        }
        ...
        public LiteFileInfo Upload(string id, string filename)
        {
            ...
            {
                return Upload(new LiteFileInfo(id, filename), stream);
            }
        }
        ...
        public LiteFileInfo Upload(string filename)
        {
            using (var stream = new FileStream(filename, FileMode.Open, FileAccess.Read))
            {
                return Upload(new LiteFileInfo(Path.GetFileName(filename), filename), stream);
            }
        }
        ...
        public LiteFileStream OpenRead(string id)
        {
            ...
            return OpenRead(new LiteFileInfo(doc));
        }
