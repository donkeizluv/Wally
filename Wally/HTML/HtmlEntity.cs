    ...
    internal class HtmlEntity
    {
        ...
        public static string DeEntitize(string text)
        {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                else if (state == ParseState.EntityStart)
                {
                    chr = text[i];
                    if (chr == '&')
                    {
                        sb.Append(string.Concat("&", entity));
                    ...
                    else
                    {
                        if (entity.Length != 0)
                        {
                            if (entity[0] != '#')
                            {
                                ...
                                if (o != null)
                                {
                                    sb.Append(Convert.ToChar((int) o));
                                }
                                else
                                {
                                    sb.Append(string.Concat("&", entity, ";"));
        ...
        /// <summary>
        ///     Clone and entitize an HtmlNode. This will affect attribute values and nodes' text. It will also entitize all child
        ///     nodes.
        /// </summary>
        ...
        ///     If set to true, the [quote], [ampersand], [lower than] and [greather than]
        ///     characters will be entitized.
        /// </param>
        /// <returns>The result text</returns>
        ...
        {
            foreach (var htmlAttribute in collection)
            {
                htmlAttribute.Value = Entitize(htmlAttribute.Value);
            }
        }
