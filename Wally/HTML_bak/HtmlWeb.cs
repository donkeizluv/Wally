...
namespace Wally.HTML
{
    ...
    internal class HtmlWeb
    {
        ...
        {
            get { return CachePath != null && _usingCache; }
            set
            {
        ...
        public string GetCachePath(Uri uri)
        {
            ...
            {
                cachePath = Path.Combine(CachePath, (uri.Host + uri.AbsolutePath).Replace('/', '\\'));
            }
            return cachePath;
        }
                                   ...
                                   ICredentials creds)
        {
            ...
            catch (WebException we)
            {
                ...
                if (resp == null)
                {
                    if (oldFile)
                    {
                        ...
                        HttpStatusCode result = HttpStatusCode.NotModified;
                        return result;
                    }
                    throw;
                }
            ...
            if (resp.StatusCode != HttpStatusCode.NotModified)
            {
                ...
                if (s != null)
                {
                    if (UsingCache)
                    {
                        SaveStream(s, cachePath, RemoveMilliseconds(resp.LastModified),
                            _streamBufferSize);
